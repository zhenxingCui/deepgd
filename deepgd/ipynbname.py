...
def _get_sessions(srv, host=None):
    ...
    try:
        ...
        srv_url = srv['url']
        if host:
            srv_url = re.sub(r'(?<=//)[^:]+(?=:)', host, srv_url)
        url = f"{srv_url}api/sessions{qry_str}"
...
def _find_nb_path(host=None) -> Union[Tuple[dict, PurePath], Tuple[None, None]]:
    try:
    ...
    for srv in _list_maybe_running_servers():
        try:
            sessions = _get_sessions(srv, host)
